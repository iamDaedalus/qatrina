...
async function companiesFunction(companyId) {
  ...
  const companyName = await fetchCompanyImages(companyId);
  const companyWrapper = document.getElementById("companyWrapper");
  const companyLanding = document.getElementById("companyLanding");
  companyWrapper.innerHTML = "";

  
  companyLanding.innerHTML = `
  <h1 class="companyLabel">${companyName.name}<h1/>`;
...
function displayCompany(data) {
  ...
  companyWrapper.appendChild(companyCard);
