...
async function networksFunction(networksId) {
 ...
 const networkName = await fetchNetworkImages(networksId);
   const networkWrapper = document.getElementById("networkWrapper");
   const networkLanding = document.getElementById("networkLanding");
   networkWrapper.innerHTML = "";

   networkLanding.innerHTML = `
  <h1 class="networkLabel">${networkName.name}<h1/>`;
...
function displayNetworks(data) {
  ...
  companyCard.innerHTML = `
      ...
      `;
  networkWrapper.appendChild(companyCard);
